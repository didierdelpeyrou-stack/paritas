/**
 * Heavy pipeline narrative content. Loaded as a dynamic chunk only when
 * a pipeline candidate first triggers — typically mid-game (turn 25+).
 * Until the chunk arrives, pipelineSceneData() in the façade returns
 * null and the engine falls through to a transition scenario.
 */

import type { Choice, RebirthGameState } from '../types';
import type { PipelineId } from './pipelineTypes';
import type { PipelineSceneData, StageBuilder } from './pipelineContent';

export const PIPELINE_STAGES: Record<PipelineId, StageBuilder[]> = {
  institution: [
    institutionStage0,
    institutionStage1,
    institutionStage2,
